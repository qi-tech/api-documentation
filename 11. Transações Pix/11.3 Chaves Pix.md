...
- endpoint: /baas/pix/list_active_keys/{account_key}
- method: GET

<br>

No endereço da requisição, no local indicado por `{account_key}` deverá ser enviada a account key da conta que será consultada.

#### Response de uma consulta de chave ativa <a name=response-list-active-keys></a>
...
- endpoint: /baas/pix/key_inclusion_request
- method: POST
- body

No corpo da requisição deve ser informada a `account_key` que receberá a pix_key.
...
#### Response de uma exclusao de chave ativa <a name="response-pixkey-exclude"></a>
