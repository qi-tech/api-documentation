...
de presentes nos exemplos, são opicionais e possibilitam ao cliente definir regras de movimentação diária para a conta.
Por exemplo, todos os dias transferir 100% do saldo da conta para uma outra conta, facilitando as operações. O objeto é 
composto por:

| Campo | Descrição | Exemplo |
|---|---|---|
| **`transfer_cronstring`** *(obrigatório)* | Email da pessoa | "api@qitech.work" |
