...
<sub><sup>
<br>1<a name=1></a>: Para encriptação da comunicação
<br>2<a name=2></a>: Para autenticação e não-repúdio da informação
...
<br>4<a name=4></a>: JWT (JSON Web Token): https://jwt.io/
<br>5<a name=5></a>: Exemplo de geração de chaves em um computador UNIX:
<br></sub></sup>
```bash
